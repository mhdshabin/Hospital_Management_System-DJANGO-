...
def dashboard(request):
    ...
    ventilator_available =ventilator.objects.filter(occupied=False).count()
    pending_request =Request.objects.filter(done=False).count()
    context = {
...
def add_patient1(request):
    ven = ventilator.objects.filter(occupied=False)
    ...
    if request.method == "POST":
        ...
        ven_num_sent = request.POST['ven_num']
        ven_num = ventilator.objects.get(ven_number=ven_num_sent)
       
     
        
        
        print(request.POST)
        ven = ventilator.objects.create(
        ...
        ven = ventilator.objects.get(ven_number=ven_num_sent)
        ven.occupied = True
        ven.save()
        id = patient.id
        return redirect(f"/patient/{id}")
    ...
    return render(request, 'add_patient1.html', context)
